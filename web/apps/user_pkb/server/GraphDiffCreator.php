...
class GraphDiffCreator{
  ...
  /**
   * Return array('originalContentId'=> , 'clonedContentId'=> , 'status'=> ). There are three types of diff nodes
   * 1. originalContentId = null, clonedContentId != null - nodes added by cloner
   * 2. originalContentId != null, clonedContentId == null - nodes deleted by cloner or added later by author of original graph (clonee)
   * 3. originalContentId != null, clonedContentId != null - nodes that stay in cloned graph and may be modified on untouched
   * @return array
   */
  public function getDiffNodes(){
    ...
    foreach($rows as $row){
      ...
      if($row['cloned_from_node_content_id'] != null){
        ...
        $combined_nodes[] = array('originalContentId'=>$row['cloned_from_node_content_id'], 'clonedContentId'=>$row['node_content_id'], 'status'=>$status);
      }
      // if it was brand new node
      else{
        $combined_nodes[] = array('originalContentId'=>null, 'clonedContentId'=>$row['node_content_id'], 'status'=>'added_by_cloner');
      }
    }
    ...
    foreach($rows as $row){
      ...
      $combined_nodes[] = array('originalContentId'=>$row['node_content_id'], 'clonedContentId'=>null, 'status'=>$status);
    }
